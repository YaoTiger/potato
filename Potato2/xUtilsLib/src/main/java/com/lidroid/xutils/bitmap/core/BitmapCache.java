...
public class BitmapCache {
	...
	private Bitmap addBitmapToMemoryCache(String uri, BitmapDisplayConfig config, Bitmap bitmap, long expiryTimestamp)
			throws IOException {
		if (config != null) {
			...
			try {
				...
				if (snapshot != null) {
					...
					if (config == null || config.isShowOriginal()) {
						...
						bitmap = BitmapDecoder.decodeSampledBitmapFromDescriptor(snapshot.getInputStream(DISK_CACHE_INDEX)
	...
	private Bitmap decodeBitmapMeta(BitmapMeta bitmapMeta, BitmapDisplayConfig config) throws IOException {
		...
		if (bitmapMeta.inputStream != null) {
			...
			if (config == null || config.isShowOriginal()) {
				...
				bitmap = BitmapDecoder.decodeSampledBitmapFromByteArray(bitmapMeta.data, config.getBitmapMaxSize(),
