package com.seable.potato.ui.activity.taskcount;
...
/**
 * @author 王维玉
 * @ClassName: Activity_Task_History
 * @Description: 历史记录
 * @date 2015-04-01 13:48
 */
public class Activity_Task_History extends LActivity implements OnItemClickListener {
    ...
    private static ArrayList<TaskInfoEntity> list;
    ...
    private void findViewById() {
        ...
        mPullRefreshListView = (PullToRefreshListView) findViewById(R.id.lv_task);
        // 设置下拉 或 上拉时显示的文字
    ...
    private void initView() {
        // 初始化view
        if (type == Constants.PARAMETER_VALUE_TASK5_HISTORY_FAILED) {
            titleBar.setTitleName(mContext.getString(R.string.main_task_history_failed));
            tabUnread = 1;
        } else {
            titleBar.setTitleName(mContext.getString(R.string.main_task_history_success));
            tabUnread = 6;
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int position,
        ...
        bundle.putString(Constants.JUMP_PARAMETER_KEY_ID, list.get(position - 1)
                .getId());
        bundle.putInt(Constants.JUMP_PARAMETER_KEY_TYPE, list.get(position - 1)
                .getTaskType());
        bundle.putInt(Constants.JUMP_PARAMETER_KEY_TASK_TYPE,
                type);
        jumpActivityBundle(Activity_Task_Detail.class,
