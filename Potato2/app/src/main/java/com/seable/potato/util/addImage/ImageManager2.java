...
import com.seable.potato.util.MD5Util;
...
public class ImageManager2 {
    ...
    private static MApplication myapp;
    ...
    public static ImageManager2 from(Context context) {
        ...
        if (myapp == null) {
            myapp = (MApplication) context.getApplicationContext();
        }
    ...
    public void displayImage(ImageView imageView, String url, int resId,
                             int width, int height) {
        System.out.println("ddddddddddd" + url);
        if (imageView == null) {
            return;
        }
        if (resId >= 0) {
...
//					System.out.println("ssss"+file);
//					String pathName =file.toString();
//					String name =pathName.substring(pathName.lastIndexOf("/")+1);
//					System.out.println("dddddsss"+name);
                        if (url.toLowerCase().contains("sdcard") || url.toLowerCase().contains("storage")) {
